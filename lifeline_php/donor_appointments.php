<?php
  ...
  if($_SESSION['status']!="Active")
  {
      header("location:login.php");
        ...
        $results = mysqli_query($conn, "SELECT apt.id as aid, apt.date, apt.time, apt.status, dr.title, dr.city, dr.id as did
                    FROM appointment apt INNER JOIN drive dr ON apt.drive_id = dr.id
                    WHERE apt.donor_id = $uid ORDER BY apt.datetime desc");
        ...
        $check = mysqli_query($conn, "SELECT COUNT(apt.id) as apptStatus, apt.date as apptDate, apt.id as aid
                                      FROM appointment apt WHERE apt.donor_id = $uid and apt.status = 'Pending'
  ...
  if (isset($_GET['del']))
    {
      ...
      echo "<script> 
        alert('$message')
        window.location.replace('donor_appointments.php');
        </script>";
      //header('location: donor_appointments.php?delete');
    }
...
<html>
    ...
    <div class="content" style="position: absolute; top:50%; transform: translateY(-56%);
            ...
            <div class="ui one cards" style="width:100%; margin:auto; z-index:0">
                    ...
                    <div class="card">
                        ...
                        <div class="header"><?php echo $fullname;?></div>
                          <div class="extra content">
                              You have an appointment set on  <?php echo $apptDate->format('m/d/Y'); ?>
                              <br><a href="donor_appointments_view.php?view=<?php echo $aid; ?>"> Click Here </a> to cancel appointment.
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <?php } else if($lastDonated != NULL AND $dateAvailable <= $dateToday)  { ?>
                    <div class="card">
                        <div class="content">
                          <div class="right floated ui image">
                        ...
                        <div class="header"><?php echo $fullname;?></div>
                            <div class="meta" >Last Donated: <?php echo $date->format('m/d/Y'); ?> </div><hr>
                          <div class="extra content">
                              You're eligible to donate!<br><a href="donor_drives.php">Click here</a> to see upcoming and ongoing Blood Drives.
                          </div>
                        </div>
                      </div>
                    </div>
                <?php }else if($lastDonated != NULL AND $dateLast < $dateAvailable){?>
                    <div class="card">
                          <div class="content">
                            <div class="right floated ui image">
                          ...
                          <div class="header"><?php echo $fullname;?></div>
                              <div class="meta" >Last Donated: <?php echo $date->format('m/d/Y'); ?> </div><hr>
                            <div class="extra content">
                                You're not yet eligible to donate.<br> Please wait until <?php echo $date4->format('m/d/Y'); ?>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <table class="ui celled unstackable table">
                  ...
                  <tbody>
                        <?php while ($row = mysqli_fetch_array($results)) { ?>
                            <tr>
                                <td data-label="Title">
                                  <a href="donor_appointments_view.php?view=<?php echo $row['aid']; ?>" target="_self">
                                    <b>
                                      <?php echo $row['title']; ?>
                                    </b>
                                  </a>
                                ...
                                <td data-label="Date Start" ><?php echo $row['date']; ?></td>
                                <td data-label="Date End" ><?php echo $row['time']; ?></td>
