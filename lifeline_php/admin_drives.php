<?php
  ...
  $result1= mysqli_query($conn, "SELECT COUNT(status) AS count FROM drive WHERE status='Pending'");
  ...
  $result3 = mysqli_query($conn, "SELECT COUNT(status) AS count FROM drive");
  $row3 = mysqli_fetch_assoc($result3); 
  $count3 = $row3['count'];

  $results = mysqli_query($conn, "SELECT drive.id, host.orgname, drive.title, drive.date_start, drive.status FROM drive INNER JOIN host ON host.id = drive.host_id WHERE drive.status NOT IN ('Rejected') AND drive.status NOT IN ('Done') ORDER BY drive.status desc");
    ...
    <div class="content" style="position: absolute; top:50%; transform: translateY(-56%);
            ...
            <div class="ui three cards" style="width:100%; margin:auto; z-index:0">
              ...
              <div class="card">
                <div class="content">
                  <div class="left floated ui image">
                    <i class="archive blue icon" style="font-size: 20px"></i> 
                  </div>
                  <div class="header">
                        <?php echo $count3; ?>
                  </div>
                  ...
                  <tbody>
                        <?php while ($row = mysqli_fetch_array($results)) { ?>
                            <tr>
                                ...
                                <?php if ($row['status'] == "Approved" || $row['status'] == "Rejected" || $row['status'] == "Done" || $row['status'] == "On Going" ){ ?>
                                    <td></td>
                                <?php } else { ?>         
                                    <td data-label="Actions" style="text-align:center">
                                        <a href="admin_drives.php?accept=<?php echo $row['id']; ?>" target="_self">
                                            <i class="check circle green icon" style="font-size: 20px"></i>
                                        </a>
                                    </td>
                                    <td data-label="Actions" style="text-align:center">
                                        <a href="admin_drives.php?decline=<?php echo $row['id']; ?>" target="_self">
